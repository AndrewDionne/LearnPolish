...
def generate_flashcard_html(set_name, data):
    ...
    Flashcards behaviour:
      - Front = English meaning, "Say it in Polish" mic button.
      - Back  = Polish phrase + pronunciation.
      - Lexical recall is the main "correct/incorrect" metric:
        • If Azure hears the correct Polish phrase before you flip → counts as correct.
        • UI shows:
            ✅ <Polish phrase>
            Pronunciation rating: <score>%
      - Pronunciation rating is based on Accuracy / Fluency / Completeness,
        with penalties for short/quiet audio and word errors.

    Difficulty (per user/profile, default = normal):
      - easy:
          • No penalties (energyF = 1, errF = 1).
      - normal:
          • Softer penalties (gentler timing + word-error factors).
      - hard:
          • Similar/stricter than previous strict mode.

    Scoring details:
      - Base metrics from Azure:
          acc, flu, comp in [0,100].
      - Penalties:
          energyF = f(voiced_ms / expected_ms)
          errF    = f(word_error_fraction)
      - For a given difficulty, penalties are applied to acc/flu/comp, then:
          strict = 0.55*acc_adj + 0.25*flu_adj + 0.20*comp_adj
      - Flashcard set "score" is % of cards where the lexical phrase was correct
        before flipping the card, not the strict PA %.

    Toggles (optional URL query):
  ...
  <script>
    ...
    window.addEventListener("DOMContentLoaded", async function() {{
      ...
      sayBtn.addEventListener("click", async function(e) {{
        ...
        const ref = getRef();
        if (!ref.trim()) {{
          frontRes.textContent = "⚠️ No reference text.";
          return;
        }}
        ...
        if (lexical) {{
          ...
          frontRes.textContent = "Pronunciation rating: " + score + "% — try again.";
        }} else {{
      ...
      if (!done) {{
        try {{
          ...
          if (raw) {{
            ...
            apply(
              Math.round(Number(j.score) || 0),
              Number(j.attempts) || 0,
              Number(j.total) || undefined,
              Number(j.points_total) || undefined,
              awarded
            );
            done = true;
          }}
