...
<body>
  ...
  <script src="https://aka.ms/csspeech/jsbrowserpackageraw"></script>
    ...
    async function getSpeechToken(forceRefresh=false) {{
      ...
      if (!forceRefresh && c && c.exp > Date.now()) return c;
      try {{
        ...
        const ttlMs  = Math.max(60_000, ((t && t.expires_in ? t.expires_in : 540) * 1000) - 30_000);
    ...
    // --- Mic pre-warm for Safari/macOS ---
    // Grants permission & spins up audio quickly, then releases the mic
    // so the SDK can grab it without clipping the first syllable.
    async function prewarmMic() {{
      try {{
        if (!navigator.mediaDevices?.getUserMedia) return;
        const stream = await navigator.mediaDevices.getUserMedia({{ audio: true }});
        stream.getTracks().forEach(tr => tr.stop());
        // Nudge AudioContext (Safari)
        try {{
