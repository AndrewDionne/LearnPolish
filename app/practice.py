...
def generate_practice_html(set_name, data):
    """
    Generates docs/practice/<set_name>/index.html.

    ✅ Preserves functionality:
      - "Repeat after me" practice loop with Pause/Resume/Restart
...
<body>
  ...
  <script>
    ...
    function playAudio(filename, callback) {{
      ...
      if (!audio) {{ console.warn("⚠️ Audio not preloaded:", filename); callback(); return; }}
      audio.currentTime = 0;
      audio.onended = callback;
      audio.onerror = () => {{ console.warn("⚠️ Audio failed:", filename); callback(); }};
      const p = audio.play();
      if (p) p.catch(err => {{ console.warn("🔇 Autoplay blocked:", err); callback(); }});
    }}
