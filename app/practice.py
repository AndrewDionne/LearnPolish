...
<body
  ...
  <script>
    ...
    function computeStrictScore(paJson, base, refText, voicedMs){{
      ...
      // Base blend from Azure metrics
      let baseScore = Math.round(0.55 * acc + 0.25 * flu + 0.20 * comp);

      // More generous timing model
      const syll     = estimateSyllablesPL(refText);
      const targetMs = Math.max(400, Math.min(2200, 320 + 130 * syll));
      const needed   = 0.65 * targetMs;
      const energyF  = Math.max(0.7, Math.min(1, (voicedMs || 0) / needed));

      // Softer word error penalty
      const errF = Math.max(0.55, 1 - 0.45 * wordErrFrac);
